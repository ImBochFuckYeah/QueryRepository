...
BEGIN
	...
	--obtenerEmpresa del contrato del emleado
	DECLARE @noEmpresa INT = (SELECT codEmpresa FROM tContrato WHERE noContract = @noContract);
    ...
    DECLARE @dias_laborados INT = (
		...
		FROM tContrato ctc
		WHERE ctc.noContract = @noContract
        --FROM tEmpleado emp
    ...
    WITH Salarios AS (
        SELECT 
            SUM((COALESCE(pnl.salarioMensual, 0) / 30) * 15) AS salario_ordinario,
            SUM(pnl.sSimples + pnl.sDobles + pnl.otrosIngresos) AS salario_extraordinario
            -- pnl.*
        FROM tPeriodo prd
        LEFT JOIN tPlanilla pnl ON prd.idPeriodo = pnl.idPeriodo
    ...
    DECLARE @fecha_inicio_aguinaldo DATE = DATEFROMPARTS(YEAR(
			CASE WHEN CAST(@fecha_calculo AS DATE) < DATEFROMPARTS(YEAR(@fecha_calculo), 12, 1)
				THEN DATEADD(YEAR, -1, @fecha_calculo) 
			ELSE @fecha_calculo END
    ...
    DECLARE @salario_proporcional_aguinaldo DECIMAL(10, 4) = (
        ...
        WHERE 
			--prd.fechaInicio >= @fecha_inicio_aguinaldo
            --AND prd.fechaFin <= @fecha_fin_aguinaldo 
            --AND 
			pnl.codEmpleado = @codigo
    ...
    --);
	DECLARE @aguinaldo DECIMAL(10,4) = CASE
		WHEN @fecha_calculo BETWEEN DATEFROMPARTS(YEAR(@fecha_calculo), 10, 16) AND DATEFROMPARTS(YEAR(@fecha_calculo), 11, 30)
			THEN 0
		ELSE (
			SELECT ISNULL(((SUM(pnl.salarioMensual / 2) + @salario_proporcional_aguinaldo) /12), 0) FROM tPlanilla pnl
			JOIN tPeriodo prd ON prd.idPeriodo = pnl.idPeriodo
			WHERE prd.fechaInicio >= @fecha_inicio_aguinaldo AND prd.fechaFin <= @fecha_fin_aguinaldo AND pnl.codEmpleado = @codigo
		)
		END;
    ...
    DECLARE @fecha_inicio_bono_catorce DATE = DATEFROMPARTS(YEAR(
			...
			CASE WHEN CAST(@fecha_calculo AS DATE) < DATEFROMPARTS(YEAR(@fecha_calculo), 7, 1) THEN DATEADD(YEAR, -1, @fecha_calculo) ELSE @fecha_calculo END
    ), 7, 1);
    ...
    --);
	DECLARE @bono_catorce DECIMAL(10, 4) = CASE 
		WHEN @fecha_calculo BETWEEN DATEFROMPARTS(YEAR(@fecha_calculo), 5, 16) AND DATEFROMPARTS(YEAR(@fecha_calculo), 6, 30)
			THEN 0
		ELSE (SELECT ISNULL(((SUM(pnl.salarioMensual / 2) + @salario_proporcional_bono_catorce) / 12), 0)
            FROM tPlanilla pnl
            JOIN tPeriodo prd ON prd.idPeriodo = pnl.idPeriodo
            WHERE prd.fechaInicio >= @fecha_inicio_bono_catorce AND prd.fechaFin <= @fecha_fin_bono_catorce AND pnl.codEmpleado = @codigo)
		END;
    ...
    DECLARE @vacaciones_proporcionales DECIMAL(10, 4) = (
        CASE 
            WHEN DATEDIFF(DAY, @ultimas_vacaciones, @fecha_calculo) / 364 > 0
            THEN 
                ((@salario_ordinario + @salario_extraordinario) / 30) * 
                (
                    DATEDIFF(DAY, DATEADD(YEAR, 1, @ultimas_vacaciones), @fecha_calculo) + 1
                ) * 0.0410958904109589
            ELSE 
                ((@salario_ordinario + @salario_extraordinario) / 30) * 
                (
                    DATEDIFF(DAY, @ultimas_vacaciones, @fecha_calculo) + 1
                ) * 0.0410958904109589
        END
    );
    ...
    SELECT 
        ...
        @indemnizacion + @aguinaldo + @bono_catorce + @vacaciones_proporcionales AS total,
        (@indemnizacion + @aguinaldo + @bono_catorce + @vacaciones_proporcionales) * 0.75 AS pasivo,
		--fecha para calculo
		@fecha_calculo AS fecha_calculo,
		@noEmpresa AS id_empresa;
END;
GO
