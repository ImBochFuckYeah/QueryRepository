...
GO
	...
	ALTER PROCEDURE [dbo].[spCalcularPlanilla] @idPeriodo as int,
	@codEmpresa as int AS DECLARE @fechaInicio date,
	@fechaFin date,
	@dias int,
...
FROM
	tPeriodo
WHERE
	idPeriodo = @idPeriodo DELETE tPlanillaTemporal
WHERE
	idPeriodo = @idPeriodo
	AND idEmpresa = @codEmpresa
INSERT INTO
	tPlanillaTemporal (
		idEmpresa,
		idPeriodo,
...
SELECT
	...
	aliasCodigo as codigo,
	nombreEmpleado + ' ' + isnull(segundoNombre, '') + ' ' + apellidoEmpleado + ' ' + isnull(segundoApellido, '') + ' ' + isnull(apellidoCasada, '') as Empleado,
	salarioOrdinario as salarioMensual,
	t1.bonifDecreto,
	diasLaborados = dbo.fnDiasLaborados(
		@fechaInicio,
		@fechaFin,
		@dias,
		t1.fechaIngreso,
	...
	isnull(
		(
			...
			WHERE
				...
				AND codEmpleado = t1.codEmpleado
		),
		0
	) as hSimple,
	isnull(
	...
	isnull(
		(
			...
			FROM
				tDescuento
			WHERE
				idTipoDescuento NOT IN(6, 17, 29)
				AND vigente = 1
				AND idPeriodo = @idPeriodo
				AND codEmpleado = t1.codEmpleado
				AND isnull(idPlanilla, 0) = 0
		),
		0
	) + isnull(
		(
...
WHERE
	(
		t1.finContract is null
		OR t1.finContract >= @fechaInicio
		OR t1.finContract = '1900-01-01'
	)
	AND t1.fechaIngreso <= @fechaFin
	AND t1.codEmpresa = @codEmpresa --AND t1.activo = 1
INSERT INTO
	tMensajeError
VALUES
	(
		'SE GENERO LA PLANILLA ' + CAST(@idPeriodo as varchar)
	);
...
ORDER BY
	departamento ASC,
	codigo ASC --Fin de consulta para generar nomina